...
#pragma warning restore CS8625
    ...
    public static void TryFormatTest()
    {
        ...
        Assert.True(dateOnly.TryFormat(buffer, out charsWritten, "o".AsSpan()));
        Assert.Equal(10, charsWritten);
        Assert.True(dateOnly.TryFormat(buffer, out charsWritten, "R".AsSpan()));
        Assert.Equal(16, charsWritten);
        Assert.False(dateOnly.TryFormat(buffer.Slice(0, 3), out charsWritten));
        Assert.False(dateOnly.TryFormat(buffer.Slice(0, 3), out charsWritten, "r".AsSpan()));
        Assert.False(dateOnly.TryFormat(buffer.Slice(0, 3), out charsWritten, "O".AsSpan()));
        Assert.Throws<FormatException>(() => {
            Span<char> buff = stackalloc char[100];
            dateOnly.TryFormat(buff, out charsWritten, "u".AsSpan());
        });
        Assert.Throws<FormatException>(() => {
            Span<char> buff = stackalloc char[100];
            dateOnly.TryFormat(buff, out charsWritten, "hh-ss".AsSpan());
        });
