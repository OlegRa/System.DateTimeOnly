...
public class TimeOnlyTests
{
    ...
    public static void ConstructorsTest()
    {
        ...
        Assert.Equal(0, to.Nanosecond);
        Assert.Equal(DateTimeExtensions.Create(1, 1, 1, to.Hour, to.Minute, to.Second, to.Millisecond, to.Microsecond).Ticks, to.Ticks);
    ...
    public static void AddTest()
    {
        ...
        Assert.Equal(Diagnostics.TimeSpan.NanosecondsPerTick, to.Nanosecond);
        to = to.Add(new TimeSpan(Diagnostics.TimeSpan.TicksPerMicrosecond));
    ...
    public static void TryFormatTest()
    {
        ...
        buffer.Fill(' ');
        Assert.True(timeOnly.TryFormat(buffer, out charsWritten, "o".AsSpan()));
        Assert.Equal(16, charsWritten);
        Assert.Equal(16, buffer.TrimEnd().Length);

        buffer.Fill(' ');
        Assert.True(timeOnly.TryFormat(buffer, out charsWritten, "R".AsSpan()));
        ...
        Assert.False(timeOnly.TryFormat(buffer.Slice(0, 3), out charsWritten, "r".AsSpan()));
        Assert.False(timeOnly.TryFormat(buffer.Slice(0, 3), out charsWritten, "O".AsSpan()));

        Assert.Throws<FormatException>(() => {
            Span<char> buff = stackalloc char[100];
            timeOnly.TryFormat(buff, out charsWritten, "u".AsSpan());
        });
        Assert.Throws<FormatException>(() => {
            Span<char> buff = stackalloc char[100];
            timeOnly.TryFormat(buff, out charsWritten, "dd-yyyy".AsSpan());
        });
